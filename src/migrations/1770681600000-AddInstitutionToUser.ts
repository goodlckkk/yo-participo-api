...
export class AddInstitutionToUser1770681600000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    try {
      ...
      await queryRunner.query(
        `ALTER TYPE "public"."users_role_enum" ADD VALUE IF NOT EXISTS 'INSTITUTION'`,
      );
    } catch (e) {
      console.log(
        '⚠️ Advertencia: No se pudo alterar el enum user_role_enum. Puede que no use enum nativo o el nombre sea diferente.',
        e.message,
      );
    }
    ...
    console.log(
      '✅ Migración: Rol INSTITUTION y campo institutionId agregados a users',
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    const foreignKey = table.foreignKeys.find(
      (fk) => fk.columnNames.indexOf('institutionId') !== -1,
    );
    if (foreignKey) {
